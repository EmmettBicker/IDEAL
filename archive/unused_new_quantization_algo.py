...
def binary_latents_to_hard_tokens(idea_latents: torch.Tensor, latent_dim: int):
    with torch.no_grad():
        hard_q = torch.sign(idea_latents)
        # Convert from {-1, 1} to {0, 1} for indexing
        binary_indices = (hard_q > 0).float()

        power_tensor = torch.pow(
            2, torch.arange(0, latent_dim, device=idea_latents.device)
        ).flip(0)
...
def binary_latents_to_token_probs(idea_latents: torch.Tensor):
    ...
    indices_ones = (
        torch.arange(vocab_size, device=device)[:, None]
        .bitwise_and(2 ** torch.arange(num_digits - 1, -1, -1, device=device))
        .bool()
        .float()
    )
