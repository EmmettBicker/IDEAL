"""
Modified from vector_quantize_pytorch's implementation
        ...
        with quantization_context():
            ...
            if self.training:
                ...
                if self.frac_per_sample_entropy < 1.:
                    # EMMETT'S CHANGED CODE
                    num_tokens = original_input.shape[0] * original_input.shape[1]  # b * n
                    ...
                    flat_input = rearrange(original_input, 'b n ... -> (b n) ...')
                    sampled_input = flat_input[rand_mask]

                    distance = -2 * einsum('... d, j d -> ... j', sampled_input, codebook)
                    prob = (-distance * inv_temperature).softmax(dim = -1)
                    per_sample_probs = prob
                    # account for mask

                    # if exists(mask): Not implemented
                    #     prob = prob[mask]
                    # else:
                    #     prob = rearrange(prob, 'b n ... -> (b n) ...')
                    # CODE THAT USED TO BE IN THE ORIGINAL IMPLEMENTATION
